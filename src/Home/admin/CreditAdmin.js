...
export default function CreditAdmin() {
  ...
  const [filter, setFilter] = useState("month"); // Filter state
  ...
  useEffect(() => {
    const fetchCredits = async () => {
      try {
        const response = await axios.get(process.env.REACT_APP_API_BASE_URL + `/credit/user/`);
        setCredits(response.data);

        const userIds = response.data.map(credit => credit.userId);
        const userResponses = await Promise.all(
          userIds.map(userId => axios.get(process.env.REACT_APP_API_BASE_URL + `/users/${userId}`))
        );
        ...
        console.error("Error fetching credits and users:", error);
      }
    };
  ...
  if (filter === "week") {
    filteredCredits = credits.filter(credit => moment(credit.createdAt).isBetween(startOfWeek, endOfWeek, null, "[]"));
  } else if (filter === "month") {
    filteredCredits = credits.filter(credit => moment(credit.createdAt).isBetween(startOfMonth, endOfMonth, null, "[]"));
  }
    ...
    <div className="flex h-screen bg-gray-50">
      ...
      <div className="ml-64 p-5 w-full">
        ...
        <div className="flex items-center space-x-4 bg-green-100 p-4 rounded-lg mb-4 shadow-md border border-green-300">
          ...
          <select className="p-2 border border-green-400 rounded-md shadow-sm transition duration-300 ease-in-out focus:ring focus:ring-green-300" 
            value={filter} 
        ...
        <table className="min-w-full bg-white border border-gray-300 shadow-md rounded">
          <thead>
            <tr className="bg-gray-200">
              ...
              <th className="py-2 px-4 border">Agence</th>
            ...
            {filteredCredits.map((credit) => {
              ...
              return (
                <tr key={credit._id} className="text-center">
                  ...
                  <td className="py-2 px-4 border">{user ? user.city : "Chargement..."}</td>
