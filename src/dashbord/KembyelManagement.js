...
const KmebyleAdmin = () => {
  ...
  const fetchTraites = async () => {
    ...
    try {
      ...
      showToast("Échec de la récupération des traites", "error");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const validateForm = () => {
    ...
    if (traiteForm.payments.some((payment) => !payment.amount || payment.amount <= 0)) {
      showToast("Tous les montants de paiement doivent être supérieurs à 0", "error");
      return false;
    }
    return true;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingTraite) {
        ...
        showToast("Traite mise à jour avec succès!", "success");
      } else {
        await axios.post(`${process.env.REACT_APP_API_BASE_URL}/kembyel`, payload);
        showToast("Traite créée avec succès!", "success");
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      showToast("Traite supprimée avec succès!", "success");
      fetchTraites();
    } catch (error) {
  ...
  const togglePaymentStatus = async (traiteId, paymentIndex) => {
    ...
    if (!traite || !traite.payments[paymentIndex]) {
      showToast("Traite ou paiement introuvable.", "error");
      return;
    }
    if (traite.payments[paymentIndex].status === "paid") {
      showToast("Ce paiement est déjà marqué comme payé.", "error");
      return;
    }

    setIsSubmitting(true);
    console.log(`Attempting to PATCH: ${process.env.REACT_APP_API_BASE_URL}/kembyel/kembyel/${traiteId}/payment/${paymentIndex}`);
    console.log("Payload:", { status: "paid" });

    try {
      await axios.patch(
        `${process.env.REACT_APP_API_BASE_URL}/kembyel/kembyel/${traiteId}/payment/${paymentIndex}`,
        { status: "paid" },
        {
          headers: {
      ...
      const errorMessage =
        error.response?.data?.message || "Échec de la mise à jour du statut de paiement";
      showToast(errorMessage, "error");
    } finally {
      setIsSubmitting(false);
    }
  ...
  const clearDateFilter = () => {
    setStartDate("");
    setEndDate("");
    showToast("Filtre de date réinitialisé", "success");
  };
